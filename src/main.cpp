...
// Copyright (c) 2013 The egcoin developers
...
uint256 hashGenesisBlock("0x000009a007787616bd5f633319728787c5ccf1736458651aa33d3138076a6675");
static const unsigned int timeGenesisBlock = 1469628721;
...
const string strMessageMagic = "egcoin Signed Message:\n";
...
static const int64 nTargetTimespan = 40 * 60; // readjust difficulty once per hour
static const int64 nTargetSpacing = 60; // 30 seconds one block
...
{
     /* 2880 blocks/day , 3153600 blocks/year */
    int64 nSubsidy = 0;
	if (nHeight == 1) 
    {
        nSubsidy = 1080000000 * COIN;
    }
	else if (nHeight > 1 )
    {
        nSubsidy = 0 * COIN;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
       const char* pszTimestamp = "egcoin 2016-7-27.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x433d9f3a524bebdbe93bb2df359788dca20c003242de35bf10306e70600ef495"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("egcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("egcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("egcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("egcoin-miner");
    ...
    try { loop {
        ...
        printf("Running egcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
